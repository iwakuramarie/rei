	...
	while ((opt = getopt(argc, argv, "hvm:t:c:r:s:a:")) != -1) {
		switch (opt) {
			case 'v':
				...
				break;
			case 'h':
				printf("rei [-h|-v|-m COUNT|-t TIMEOUT|-c CONFIG_FILE|-r REDIR_FILE|-s STATUS_FIFO|-a ABORT_KEYSYM] [EXTRA_CONFIG ...]\n");
				exit(EXIT_SUCCESS);
				break;
			case 'm':
...
void setup(void)
{
	...
	root = screen->root;
	if ((shell = getenv(REI_SHELL_ENV)) == NULL && (shell = getenv(SHELL_ENV)) == NULL)
